...
public abstract class TransactionAspectSupport implements BeanFactoryAware, InitializingBean {
	 ...
	 *
	 * @param transactionAttributes key:方法名 value：逗号分隔的事务属性
	 * @see #setTransactionAttributeSource
	...
	protected Object invokeWithinTransaction(Method method, @Nullable Class<?> targetClass,
		...
		// TransactionManager 只能为 ReactiveTransactionManager 或 PlatformTransactionManager
	...
	protected TransactionManager determineTransactionManager(@Nullable TransactionAttribute txAttr) {
		...
		if (txAttr == null || this.beanFactory == null) {
			// 方法无对应的事务属性元数据，使用已经配置的 TransactionManager
			return getTransactionManager();
		}
		// @Transactional 指定的事务管理器 bean 名称
	...
	private TransactionManager determineQualifiedTransactionManager(BeanFactory beanFactory, String qualifier) {
		...
		if (txManager == null) {
			// 优先从缓存获取，缓存未命中则从 BeanFactory 获取，然后放到缓存中
	...
	/**
	 * 将对象转换为 PlatformTransactionManager
	 *
	...
	protected TransactionInfo createTransactionIfNecessary(@Nullable PlatformTransactionManager tm,
		...
		if (txAttr != null) {
			if (tm != null) {
				...
				// 不存在 @Transactional 注解，也不存在事务管理器，将忽略事务
	...
	protected void completeTransactionAfterThrowing(@Nullable TransactionInfo txInfo, Throwable ex) {
		if (txInfo != null && txInfo.getTransactionStatus() != null) {
			...
			if (txInfo.transactionAttribute != null && txInfo.transactionAttribute.rollbackOn(ex)) {
				...
				try {
					// 继续提交事务，当前异常不需要回滚，事务状态设置为仅回滚时继续回滚
					txInfo.getTransactionManager().commit(txInfo.getTransactionStatus());
				} catch (TransactionSystemException ex2) {
	...
	protected static final class TransactionInfo {
		...
		/**
		 * 根据 TransactionAttribute 获取到的新的事务状态
		 */
		@Nullable
