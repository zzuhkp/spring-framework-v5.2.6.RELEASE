...
public class DependencyDescriptor extends InjectionPoint implements Serializable {
	...
	/**
	 * 在急切地解析潜在的目标bean以进行类型匹配的意义上，这个依赖是否是“eager”
	 */
	private final boolean eager;

	/**
	 * 当前依赖表示的成员变量或方法参数中泛型类型的嵌套级别
	 * 例如List<String>,如果为2则表示String类型
	 */
	...
	public boolean isRequired() {
		...
		if (this.field != null) {
			//字段是否为Optional类型，或者是否有Nullable注解或Kotlin语言的可选类型
