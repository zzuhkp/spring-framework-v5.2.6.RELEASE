...
class TypeConverterDelegate {
	...
	public <T> T convertIfNecessary(@Nullable String propertyName, @Nullable Object oldValue, @Nullable Object newValue,
		...
		// 存在自定义PropertyEditor或给定的值和所需的类型不匹配，进行类型转换
		if (editor != null || (requiredType != null && !ClassUtils.isAssignableValue(requiredType, convertedValue))) {
			// 所需类型是Collection，给定的值是String
			...
			if (editor == null) {
				//自定义PropertyEditor不存在，查找默认PropertyEditor
		...
		if (requiredType != null) {
			...
			if (convertedValue != null) {
				if (Object.class == requiredType) {
					...
					if (convertedValue instanceof String && Enum.class
							.isAssignableFrom(requiredType.getComponentType())) {
						// 字符串 -> 字符串数组
						convertedValue = StringUtils.commaDelimitedListToStringArray((String) convertedValue);
					}
	...
	private Object doConvertValue(@Nullable Object oldValue, @Nullable Object newValue,
		...
		if (requiredType != null && !requiredType.isArray() && convertedValue instanceof String[]) {
			...
			// 前面没有PropertyEditor转换字符串数组到给定的类型，转换字符串数组为逗号分隔的字符串，CSV字符串会传递到setAsText方法。
			if (logger.isTraceEnabled()) {
