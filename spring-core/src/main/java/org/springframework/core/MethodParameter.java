...
public class MethodParameter {
	...
	/**
	 * 嵌套级别；一般为1，如果大于1则表示泛型中的具体类型
	 * 对于 Map<String,List<Integer>>，1表示Map,2表示String和List,3表示Integer
	 */
	 ...
	 * 如果想表示String,可设置nestingLevel=2,typeIndexesPerLevel=0
	 * 如果想表示List,可设置nestingLevel=2,typeIndexesPerLevel=1
	 * 如果想表示Integer,可设置nestingLevel=3
	...
	/**
	 * 嵌套的方法参数，如Map<String,Object>，该字段可以表示Object
	 */
	@Nullable
	 ...
	 * @param parameterIndex 参数索引，-1为返回值，0是第一个参数，1是第二个参数，以此类推
	 *                       the index of the parameter: -1 for the method
	 *                       return type; 0 for the first method parameter; 1 for the second method
	 *                       parameter, etc.
	 * @param nestingLevel   目标类型的嵌套级别，通常为1，对于List<List>,1表明嵌套的List,2表示嵌套List的元素
	 ...
	 * @param parameterIndex 参数索引
	 *                       the index of the parameter
	 * @param nestingLevel   目标类型的嵌套级别，通常为1；
	 *                       对于List<List>,1表示嵌套的List,2表示嵌套List的元素
	 *                       the nesting level of the target type
	 *                       (typically 1; e.g. in case of a List of Lists, 1 would indicate the
	 *                       nested List, whereas 2 would indicate the element of the nested List)
	 */
	...
	/**
	 * 复制构造器，结果是基于原对象相同元数据和缓存状态的独立MethodParameter对象
	 * Copy constructor, resulting in an independent MethodParameter object
	 * based on the same metadata and cache state that the original object was in.
	 *
	 * @param original 要复制的源MethodParameter
