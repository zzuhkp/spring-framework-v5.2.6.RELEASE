...
/**
 * 在AnnotatedElement的注解层次结构中搜索相关注解的扫描器。
 * <p>
 * Scanner to search for relevant annotations in the annotation hierarchy of an {@link AnnotatedElement}.
 *
 * @author Phillip Webb
 * @author Sam Brannen
...
abstract class AnnotationsScanner {
	 ...
	 * @param searchStrategy the search strategy to use
	 * @param processor      the processor that receives the annotations
	 * @param classFilter    an optional filter that can be used to entirely filter out a specific class from the
	 *                       hierarchy
	 * @return the result of {@link AnnotationsProcessor#finish(Object)}
	 */
	@Nullable
	...
	private static <C, R> R processClassInheritedAnnotations(C context, Class<?> source,
		...
		try {
			...
			// 当前类定义的注解及通过 @Inherited 从父类继承的注解
			Annotation[] relevant = null;
			// relevant 剩下未处理的注解数量，可能包含当前类的注解或通过 @Inherited 从父类继承的注解
			...
			while (source != null && source != Object.class && remaining > 0 &&
				...
				// 这里从当前类向父类进行递归，每次只处理当前类定义的注解，
				// relevant 中子类已经处理过的注解在父类中不会再处理
					...
					if (declaredAnnotations[i] != null) {
						...
						for (int relevantIndex = 0; relevantIndex < relevant.length; relevantIndex++) {
							if (relevant[relevantIndex] != null &&
									declaredAnnotations[i].annotationType() == relevant[relevantIndex]
											.annotationType()) {
								isRelevant = true;
	...
	/**
	 * 处理直接标注的注解和父类中及接口中定义的注解
	 *
	 * @param context
	 * @param source
	 * @param processor
	 * @param classFilter
	 * @param includeInterfaces 是否搜索接口中的注解
	 * @param includeEnclosing  是否搜索外部类中的注解
	 * @param <C>
	 * @param <R>
	 * @return
	 */
	...
	/**
	 * 处理方法上的注解，包括重写的父类中的方法上的注解
	 *
	 * @param context
	 * @param aggregateIndex
	 * @param sourceClass       rootMethod 所在定义的类或父类
	 * @param processor
	 * @param classFilter
	 * @param rootMethod        要处理的方法
		...
		try {
			...
			if (sourceClass == rootMethod.getDeclaringClass()) {
				...
				for (Method candidateMethod : getBaseTypeMethods(context, sourceClass, classFilter)) {
					// 父类定义的方法不为null,并且子类定义的方法重写了父类中的方法
