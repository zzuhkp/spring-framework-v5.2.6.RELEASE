...
/**
 * 为单个源注解类型提供AnnotationTypeMapping信息。
 * 对所有元注解执行递归宽度优先爬网，以便最终提供映射根注解属性的快速方法。
 * 支持基于约定的元注解合并，以及隐式和显式@AliasFor。
 * 还提供有关镜像属性的信息。 这个类被设计为缓存，这样元注解只需要搜索一次，而不管它们实际使用了多少次。
 * <p>
...
final class AnnotationTypeMappings {
	...
	/**
	 * 添加注解及元注解对应的 AnnotationTypeMapping 到 mappings
	...
	/**
	 * 为指定的注解类型创建AnnotationTypeMappings。
	 * <p>
	...
	/**
	 * 为指定的批注类型创建AnnotationTypeMappings。
	 * <p>
	 * Create {@link AnnotationTypeMappings} for the specified annotation type.
	 *
	 * @param annotationType   the source annotation type
