...
/**
 * 帮助程序类，允许指定以声明方式调用的方法，无论它是静态的还是非静态的。
 * 用法：指定“targetClass/”targetMethod“或”targetObject/”targetMethod“，可选指定参数，准备调用程序。
...
public class MethodInvoker {
	...
	public void prepare() throws ClassNotFoundException, NoSuchMethodException {
		if (this.staticMethod != null) {
			...
			if (lastDotIndex == -1 || lastDotIndex == this.staticMethod.length()) {
				//TODO lastDotIndex == this.staticMethod.length() 什么时候成立？
	...
	/**
	 * 判断候选方法的声明参数类型与该方法应使用的特定参数列表之间匹配的算法。
	 * 确定表示类型和参数之间的类层次结构差异的权重。
	 * 直接匹配，即类型Integer->类Integer的arg，不会增加结果-所有直接匹配都表示权重0。
	 * 类型Object和类Integer的arg之间的匹配将使权重增加2，因为超类2在层次结构（即Object）中是最后一个仍然匹配所需类型对象的。
	 * Number类型和Integer类将相应地增加1的权重，因为超类1向上提升层次结构（即数字）仍然匹配所需的类型号。
	 * 因此，如果arg类型为Integer，则构造函数（Integer）将优先于构造函数（Number），后者将优先于构造函数（Object）。所有参数权重累积。
	 * 注意：这是MethodInvoker本身使用的算法，也是Spring的bean容器中用于构造函数和工厂方法选择的算法
	 * （在对常规bean定义默认的宽松构造函数解析的情况下）。
	 * <p>
